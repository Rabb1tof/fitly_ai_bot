...
public sealed class SettingsQuietHoursCallbackHandler : CallbackCommandHandlerBase
{
    public SettingsQuietHoursCallbackHandler(ILogger<SettingsQuietHoursCallbackHandler> logger)
        : base(logger)
    {
    }
    ...
    protected override bool CanHandle(CallbackQuery callbackQuery, ConversationContext session)
        => callbackQuery.Data == TelegramCommandNames.CallbackSettingsQuietHours;

    protected override Task HandleCallbackAsync(CommandContext context, CallbackQuery callbackQuery)
        => SettingsWorkflow.ShowQuietHoursMenuAsync(context);
}
